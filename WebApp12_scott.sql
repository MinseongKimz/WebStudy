...
--○ 전체 리스트 조회 쿼리문 구성
--(99.VER)
SELECT M.SID, M.NAME, M.TEL, S.KOR, S.MAT, S.ENG
FROM TBL_MEMBER M LEFT JOIN TBL_MEMBERSCORE S
ON S.SID = M.SID;
/*
1	이윤태	010-1111-1111	91	71	81
2	오이삭	010-2222-2222	80	60	70
3	신시은	010-3333-3333			
4	최문정	010-4444-4444			
5	이호석	010-5555-5555			
*/
...
SELECT M.SID, M.NAME, M.TEL
     ...
     , NVL(S.ENG, -1) AS ENG
     , NVL(S.MAT, -1) AS MAT
     , RANK() OVER(ORDER BY (NVL(S.KOR, -1)+NVL(S.ENG, -1)+NVL(S.MAT, -1)) DESC) AS RANK
FROM TBL_MEMBER M, TBL_MEMBERSCORE S
WHERE M.SID = S.SID(+);
--==>>View VIEW_MEMBERSCORE이(가) 생성되었습니다.

--○ 뷰(VIEW_MEMBERSCORE)확인
...
-- MEMBERSCORE 에서 참조 하지 않은 사람을 보면
SELECT COUNT(*) AS COUNT FROM TBL_MEMBERSCORE WHERE SID=3
;
--==> 0
-- 삭제 가능
